...
class Offset_Side_Panel extends Widget_Base {
	...
	protected function register_controls() {
		...
		// Optional legacy template source (kept for backward compatibility) and direct content like Popup widget.
		$templates = array( '' => __( '— None (use content below) —', 'dinopack-for-elementor' ) );
		...
		$this->add_control(
			'template_id',
			array(
				'label'       => esc_html__( 'Side Panel template (optional)', 'dinopack-for-elementor' ),
				'type'        => Controls_Manager::SELECT,
				'options'     => $templates,
				'default'     => '',
				'description' => esc_html__( 'Free version: you can leave this empty and use the Panel Content field below instead of a template.', 'dinopack-for-elementor' ),
			)
		);
		$this->add_control(
			'panel_content',
			array(
				'label'       => esc_html__( 'Panel content', 'dinopack-for-elementor' ),
				'type'        => Controls_Manager::WYSIWYG,
				'default'     => esc_html__( 'Add your side panel content here.', 'dinopack-for-elementor' ),
				'placeholder' => esc_html__( 'This content will appear inside the side panel.', 'dinopack-for-elementor' ),
				'description' => esc_html__( 'Used when no Side Panel template is selected above.', 'dinopack-for-elementor' ),
			)
		);
		$this->add_control(
			'panel_id',
		...
		$this->add_responsive_control(
			'panel_padding',
			array(
				'label'      => esc_html__( 'Inner padding', 'dinopack-for-elementor' ),
				...
				'selectors'  => array(
					'{{WRAPPER}} .dinopack-offset-side-panel__panel-inner' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .dinopack-offset-side-panel__panel-inner' => 'color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'size_units' => array( 'px', '%', 'em' ),
				'selectors'  => array(
					'{{WRAPPER}} .dinopack-offset-side-panel__panel' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
	...
	protected function render() {
		...
		// In Elementor editor or preview iframe, show the same placeholder as content_template
		// so the widget always looks the same in the canvas (no grey box after deselect).
		if ( class_exists( '\Elementor\Plugin' ) ) {
			$plugin     = \Elementor\Plugin::instance();
			$is_preview = $plugin->preview && $plugin->preview->is_preview_mode();
			$is_editor  = $plugin->editor && $plugin->editor->is_edit_mode();
			...
			<div class="dinopack-offset-side-panel__panel" style="<?php echo esc_attr( $panel_inline ); ?>">
				...
				<div class="dinopack-offset-side-panel__panel-inner">
					...
					if ( $template_id && class_exists( '\Elementor\Plugin' ) ) {
						...
						echo wp_kses_post( $s['panel_content'] );
					}
					?>
				</div>
			</div>
